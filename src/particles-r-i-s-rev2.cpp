...
bool readParameters(const std::string& filename, std::unordered_map<std::string, double>& params, char delimiter = ',') {
    ...
    while (std::getline(file, line)) {
        ...
        if (std::getline(ss, key, delimiter)) {
            key = trim(key); // Remove whitespace from key

            if (!(ss >> value)) { // Check if value extraction was successful
                std::cerr << "Warning: Invalid number on line " << lineNumber << " in file " << filename << ": '" << line << "'\n";
                continue;
            }

            if (params.find(key) != params.end()) {
                std::cerr << "Warning: Duplicate parameter '" << key << "' on line " << lineNumber << ". Using last occurrence.\n";
            }
            params[key] = value;
        } else {
            std::cerr << "Warning: Skipping invalid or incomplete line " << lineNumber << " in file " << filename << ": '" << line << "'\n";
        }
    }
