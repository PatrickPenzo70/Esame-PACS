...
struct MPMThermoSim{
    ...
    particles_t pt;              // particelle (x,y,vx,vy,T,phi, ecc.)
    ...
    MPMThermoSim(const Params& p): P(p), g(0), pt(0, {"id"}, {"T","phi","vx","vy"}, grid, {}, {}){
        // 1) griglia
        ...
        pt = particles_t(P.nParticles, {"id"}, {"T","phi","vx","vy"}, grid, x, y);
        for (idx_t i=0;i<pt.size();++i){ pt.dprops["T"][i]=300.0; pt.dprops["phi"][i]=0.0; pt.dprops["vx"][i]=0.0; pt.dprops["vy"][i]=0.0; }
    ...
    void p2g_mech(){
        ...
        for(long long p=0; p<(long long)pt.size(); ++p){
            // cella base
    ...
    void set_thermal_properties(){
        ...
        for(long long id=0; id<(long long)mask.size(); ++id){
            ...
            if(f>0){
                double cp = (1.0-ph)*P.cp_s + ph*P.cp_l;
                double kcr= (1.0-ph)*P.k_s  + ph*P.k_l;
    ...
    void thermal_step(double dt){
        ...
        // cinetica fusione + sorgente latente
        #pragma omp parallel for
        for(long long id=0; id<(long long)T.size(); ++id){
            if(mask[id]>0 && T[id]>P.Tm && phi[id]<1.0){
                double rate = P.Kphi * (T[id]-P.Tm) * (1.0 - phi[id]);
                rate = std::min(rate, (1.0 - phi[id])/dt);
                S[id] = - (P.rho * mask[id]) * P.Lm * rate;
                phi[id] = std::clamp(phi[id] + dt*rate, 0.0, 1.0);
            }
        }
    ...
    void run(){
        ...
        for(int s=0; s<P.steps; ++s){
            ...
            if(s % P.output_every == 0){
                double Tavg=0, phim=0; for(idx_t p=0;p<pt.size();++p){ Tavg+=pt.dprops["T"][p]; phim+=pt.dprops["phi"][p]; }
                Tavg/=std::max<idx_t>(1,pt.size()); phim/=std::max<idx_t>(1,pt.size());
